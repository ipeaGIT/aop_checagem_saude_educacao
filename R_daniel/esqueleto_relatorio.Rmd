...
params:
  ...
  arquivo_relacao: arquivo_relacao.csv
---
...
```{r}
suppressWarnings(
  cnpjs <- data.table::fread(params$arquivo_cnpjs, colClasses = c(id_estab = "character"))
)
...
```{r}
suppressWarnings(
  relacao <- data.table::fread(params$arquivo_relacao, colClasses = c(id_estab = "character"))
)

if (nrow(relacao) == 0) {
  
  relacao <- data.table::data.table(
    id_estab = character(), 
...
knitr::kable(relacao, format = "html") %>% 
  kableExtra::kable_styling(full_width = FALSE, position = "left")
